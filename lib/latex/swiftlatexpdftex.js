...
function writeFileRoutine(filename, content) {
	try {
		...
		var dir = filename.lastIndexOf("/") !== -1 ? filename.substring(0, filename.lastIndexOf("/")) : "";
		if (dir) {
			try {
				FS.mkdirTree(WORKROOT + "/" + dir);
			} catch (e) {
...
self["onmessage"] = function(ev) {
	let data = ev["data"];
	let cmd = data["cmd"];
	if (ev.data && ev.data.type === 'wasmPath') {
		wasmBinaryFile = ev.data.WASM_PATH;
			// Wait for WASM to be ready before calling run()
...
function kpse_find_file_impl(nameptr, format, _mustexist) {
	...
	if (reqname.includes("/")) {
		console.log(`[kpse_find_file_impl] Ignoring file with slash: ${reqname}`);
		return 0;
	}
	const cacheKey = format + "/" + reqname;
	if (cacheKey in texlive404_cache) {
		console.log(`[kpse_find_file_impl] ${cacheKey} found in 404 cache, skipping download.`);
		return 0;
	}
	if (cacheKey in texlive200_cache) {
		const savepath = texlive200_cache[cacheKey];
		console.log(`[kpse_find_file_impl] ${cacheKey} found in in-memory cache at ${savepath}.`);
		return allocate(intArrayFromString(savepath), "i8", ALLOC_NORMAL);
	}
	// Synchronous IndexedDB is not possible, so rely on preloaded FS
	// If not found, fallback to network
	const remote_url = self.texlive_endpoint + "pdftex/" + cacheKey;
	let xhr = new XMLHttpRequest;
	xhr.open("GET", remote_url, false);
	xhr.timeout = 15e4;
	xhr.responseType = "arraybuffer";
	console.log(`[kpse_find_file_impl] Start downloading texlive file ${remote_url}`);
	try {
		xhr.send();
	} catch (err) {
		console.log(`[kpse_find_file_impl] TexLive Download Failed ${remote_url}`);
		// Store not found marker in IDB asynchronously
		idbPutFile(cacheKey, { notFound: true, downloadedAt: new Date() });
		return 0;
	}
	if (xhr.status === 200) {
		...
		console.log(`[kpse_find_file_impl] Downloaded and cached ${cacheKey} at ${savepath}.`);
		...
		console.log(`[kpse_find_file_impl] TexLive File not exists ${remote_url}`);
...
function kpse_find_pk_impl(nameptr, dpi) {
	...
	try {
		...
		// Store not found marker in IDB asynchronously
		idbPutFile("pk-" + cacheKey, { notFound: true, downloadedAt: new Date() });
		return 0;
	}
	if (xhr.status === 200) {
		let arraybuffer = xhr.response;
		...
		// Store in IndexedDB asynchronously (prefix key to avoid collision with file cache)
		idbPutFile("pk-" + cacheKey, {
			content: arraybuffer,
			downloadedAt: new Date(),
			pkid: pkid
		});
...
const TEXLIVE_IDB_DBNAME = "texlive-cache";
const TEXLIVE_IDB_STORE = "files";
const TEXLIVE_IDB_VERSION = 1;
const TEXLIVE_CACHE_MAX_AGE_DAYS = 30; // Invalidate cache after 30 days

function openTexliveDB() {
    return new Promise((resolve, reject) => {
        const request = indexedDB.open(TEXLIVE_IDB_DBNAME, TEXLIVE_IDB_VERSION);
        request.onupgradeneeded = function(event) {
            const db = event.target.result;
            if (!db.objectStoreNames.contains(TEXLIVE_IDB_STORE)) {
                db.createObjectStore(TEXLIVE_IDB_STORE);
            }
        };
...
function idbGetFile(cacheKey) {
    return openTexliveDB().then(db => {
        return new Promise((resolve, reject) => {
            const tx = db.transaction(TEXLIVE_IDB_STORE, "readonly");
            const store = tx.objectStore(TEXLIVE_IDB_STORE);
...
function idbPutFile(cacheKey, obj) {
    return openTexliveDB().then(db => {
        return new Promise((resolve, reject) => {
            const tx = db.transaction(TEXLIVE_IDB_STORE, "readwrite");
            const store = tx.objectStore(TEXLIVE_IDB_STORE);
            const req = store.put(obj, cacheKey);
...
async function preloadTexliveCacheToFS() {
    try {
        const db = await openTexliveDB();
        const tx = db.transaction(TEXLIVE_IDB_STORE, "readonly");
        const store = tx.objectStore(TEXLIVE_IDB_STORE);
        ...
        return new Promise(resolve => {
            req.onsuccess = function(event) {
                ...
                if (cursor) {
                    ...
                    const { content, fileid, downloadedAt, notFound } = cursor.value;
                    // Cache invalidation: skip if too old
                    if (downloadedAt && daysBetween(new Date(downloadedAt), new Date()) > TEXLIVE_CACHE_MAX_AGE_DAYS) {
                        console.log(`[preloadTexliveCacheToFS] Skipping expired cacheKey: ${cacheKey}`);
                        cursor.continue();
                        return;
                    }
                    if (notFound) {
                        texlive404_cache[cacheKey] = 1;
                        console.log(`[preloadTexliveCacheToFS] Marked 404 for cacheKey: ${cacheKey}`);
                        cursor.continue();
                        return;
                    }
                    if (content && fileid) {
                        ...
                        try {
                            ...
                            texlive200_cache[cacheKey] = savepath;
                            console.log(`[preloadTexliveCacheToFS] Loaded ${cacheKey} into MEMFS at ${savepath}`);
                        } catch (e) {
                            console.warn(`[preloadTexliveCacheToFS] Failed to write ${cacheKey} to MEMFS:`, e);
                        }
                    }
                    cursor.continue();
                } else {
            ...
            req.onerror = function() { 
                console.warn('[preloadTexliveCacheToFS] IndexedDB cursor error');
                resolve(); 
            };
        });
    } catch (e) {
        console.warn('[preloadTexliveCacheToFS] IndexedDB open error', e);
    }
}

// Preload cache before any compilation
if (typeof ENVIRONMENT_IS_WORKER !== 'undefined' && ENVIRONMENT_IS_WORKER) {
    // Only do this in worker context
    addOnPreRun(function() {
        addRunDependency('preload-texlive-cache');
        preloadTexliveCacheToFS().then(function() {
            removeRunDependency('preload-texlive-cache');
        });
    });
}
